    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 18px", background: done ? "rgba(59,130,246,0.06)" : error ? "rgba(239,68,68,0.04)" : "transparent", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
          {done && (
            <button onClick={copy} style={{ padding: "5px 12px", fontSize: "11px", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: copied ? "#10b981" : "rgba(226,232,240,0.5)", cursor: "pointer", whiteSpace: "nowrap" }}>
          ...
          {isCustom && (
            <button onClick={onDelete} style={{ padding: "5px 10px", fontSize: "13px", borderRadius: "6px", border: "1px solid rgba(239,68,68,0.3)", background: "transparent", color: "#ef4444", cursor: "pointer" }}>✕</button>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  const generateOne = useCallback(async (sectionId, secs) => {
    const sec = (secs || FSI_SECTIONS).find(s => s.id === sectionId);
    if (!sec) return;
    const ud = userDataRef.current;
    const grant = FSI_GRANTS.find(g => g.id === grantRef.current);
...
- Только текст раздела без заголовка и нумерации`
  ...
  useEffect(() => {
    ...
    const run = async () => {
      for (const s of FSI_SECTIONS) {
        await generateOne(s.id, FSI_SECTIONS);
      }
      setAllDone(true);
  ...
  const exportText = () => {
    ...
    a.download = "fsi_grant.txt";
    a.click();
  };

  const btn = (text, onClick, disabled, color) => (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      color: disabled ? "rgba(226,232,240,0.3)" : "#fff",
      boxShadow: disabled ? "none" : `0 0 20px ${color || "#3b82f6"}44`,
    }}>{text}</button>
  );
      ...
      <div style={{ maxWidth: "780px", margin: "0 auto", padding: "40px 20px" }}>
        ...
        <div style={{ marginBottom: "28px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
            ...
            <span style={{ fontSize: "11px", fontFamily: "monospace", color: "rgba(226,232,240,0.4)", letterSpacing: "0.1em" }}>ФОНД СОДЕЙСТВИЯ ИННОВАЦИЯМ · AI ГЕНЕРАТОР</span>
          </div>
          <h1 style={{ margin: 0, fontSize: "26px", fontWeight: "700", lineHeight: "1.25" }}>
            Генератор заявки ФСИ<br />
            <span style={{ color: "#3b82f6" }}>заполни грант за 10 минут</span>
          </h1>
        </div>

        {/* Steps */}
        <div style={{ display: "flex", gap: "6px", marginBottom: "28px" }}>
          {["Выбор гранта", "Твои данные", "Результат"].map((s, i) => (
            <div key={i} style={{
              flex: 1, padding: "8px", borderRadius: "8px", textAlign: "center",
              fontSize: "11px", fontWeight: "600",
              background: step === i ? "rgba(59,130,246,0.15)" : "transparent",
              border: `1px solid ${step === i ? "#3b82f6" : "rgba(255,255,255,0.07)"}`,
              color: step === i ? "#3b82f6" : "rgba(226,232,240,0.4)",
            }}>{s}</div>
          ))}
        </div>
        ...
        {step === 0 && (
          <div>
            <p style={{ margin: "0 0 16px", fontSize: "13px", color: "rgba(226,232,240,0.5)" }}>Выбери программу ФСИ:</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "24px" }}>
              {FSI_GRANTS.map(g => (
                <div key={g.id} onClick={() => g.ready && setSelectedGrant(g.id)} style={{
                  padding: "16px", borderRadius: "12px", position: "relative",
                  cursor: g.ready ? "pointer" : "default", opacity: g.ready ? 1 : 0.4,
                  border: `1px solid ${selectedGrant === g.id ? "#3b82f6" : g.ready ? "rgba(255,255,255,0.07)" : "rgba(255,255,255,0.04)"}`,
                  background: selectedGrant === g.id ? "rgba(59,130,246,0.15)" : "#13161e",
                  ...
                  {selectedGrant === g.id && <span style={{ position: "absolute", top: "10px", right: "12px", color: "#3b82f6" }}>✓</span>}
            ...
            {selectedGrant === "student_startup" && (
              <div style={{ marginBottom: "24px" }}>
                <p style={{ margin: "0 0 12px", fontSize: "13px", color: "rgba(226,232,240,0.5)" }}>Выбери направление:</p>
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {DIRECTIONS.map(d => (
                    <div key={d.id} onClick={() => setSelectedDir(d.id)} style={{
                      display: "flex", alignItems: "center", gap: "12px",
                      padding: "12px 16px", borderRadius: "10px", cursor: "pointer",
                      border: `1px solid ${selectedDir === d.id ? "#3b82f6" : "rgba(255,255,255,0.07)"}`,
                      background: selectedDir === d.id ? "rgba(59,130,246,0.15)" : "#13161e",
                      transition: "all 0.15s",
                    }}>
                      ...
                      <div style={{ fontSize: "13px", color: selectedDir === d.id ? "#e2e8f0" : "rgba(226,232,240,0.5)", fontWeight: selectedDir === d.id ? "600" : "400" }}>{d.full}</div>
            ...
            {btn(
              ...
              () => { if (selectedGrant && (selectedGrant !== "student_startup" || selectedDir)) { grantRef.current = selectedGrant; dirRef.current = selectedDir; setStep(1); } },
              !selectedGrant || (selectedGrant === "student_startup" && !selectedDir)
            )}
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            ...
            {btn("⚡ Сгенерировать заявку", () => {
              if (!canProceed) return;
              userDataRef.current = { ...userData };
              grantRef.current = selectedGrant;
              dirRef.current = selectedDir;
              setAllDone(false);
              setResults({});
              generatingRef.current = false;
              setStep(2);
        ...
        {step === 2 && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#13161e", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "10px", padding: "12px 16px", marginBottom: "20px" }}>
              <div>
                <div style={{ fontSize: "13px", fontWeight: "600" }}>{userData.title}</div>
            ...
            {/* Прогресс */}
            <div style={{ background: "#13161e", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "10px", padding: "14px 18px", marginBottom: "24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                <span style={{ fontSize: "13px", color: allDone ? "#10b981" : "#3b82f6", fontWeight: "600" }}>
                  {allDone ? `✓ Все ${sections.length} разделов готовы!` : `Генерирую... ${doneCount} / ${sections.length}`}
                </span>
                {allDone && (
                  <button onClick={exportText} style={{ padding: "6px 16px", borderRadius: "6px", border: "none", background: "#10b981", color: "#fff", fontWeight: "600", fontSize: "12px", cursor: "pointer" }}>↓ Скачать .txt</button>
                )}
              </div>
              <div style={{ height: "5px", borderRadius: "3px", background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(doneCount / sections.length) * 100}%`, background: allDone ? "#10b981" : "#3b82f6", transition: "width 0.4s" }} />
              </div>
            </div>

            {/* Карточки — результат виден сразу */}
            {sections.map(s => (
              <SectionCard
                key={s.id} section={s}
            ...
            {allDone && !showAddField && (
              <button onClick={() => setShowAddField(true)} style={{ width: "100%", padding: "11px", borderRadius: "10px", marginTop: "4px", border: "1px dashed rgba(245,158,11,0.4)", background: "transparent", color: "#f59e0b", fontSize: "13px", cursor: "pointer" }}>
                + Добавить своё поле
              </button>
            )}
            ...
            {showAddField && (
              <div style={{ background: "rgba(245,158,11,0.05)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: "12px", padding: "18px", marginTop: "8px" }}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <button onClick={addAndGenerate} disabled={!newLabel.trim()} style={{ flex: 1, padding: "10px", borderRadius: "8px", border: "none", background: newLabel.trim() ? "#f59e0b" : "rgba(255,255,255,0.05)", color: newLabel.trim() ? "#000" : "rgba(226,232,240,0.3)", fontWeight: "600", fontSize: "13px", cursor: newLabel.trim() ? "pointer" : "not-allowed" }}>
            ...
            {allDone && (
              <div style={{ marginTop: "24px", padding: "22px", borderRadius: "12px", background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.2)", textAlign: "center" }}>
                <div style={{ fontSize: "15px", fontWeight: "700", color: "#10b981", marginBottom: "6px" }}>✓ Заявка готова!</div>
                <div style={{ fontSize: "12px", color: "rgba(226,232,240,0.4)", marginBottom: "16px" }}>↺ — перегенерировать раздел · "копировать" — скопировать текст</div>
                <button onClick={exportText} style={{ padding: "12px 36px", borderRadius: "8px", border: "none", background: "#10b981", color: "#fff", fontWeight: "700", fontSize: "14px", cursor: "pointer" }}>↓ Скачать всю заявку .txt</button>
              </div>
            )}
          </div>
        )}

        <div style={{ marginTop: "40px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.05)", fontSize: "11px", color: "rgba(226,232,240,0.1)", textAlign: "center" }}>
